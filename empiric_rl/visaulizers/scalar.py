...
def _get_dataframes(log_dir):
    with open(os.path.join(log_dir, "progress.csv"), "r") as file:
        return pd.read_csv(file)


def load_dataframes_from_tune(tune_dir):
    if not os.path.exists(tune_dir):
        raise FileNotFoundError("Logs directory does not exist")

    log_dirs = [name for name in os.listdir(tune_dir)
                if os.path.isdir(os.path.join(tune_dir, name))]
    if len(log_dirs) == 0:
        FileNotFoundError("Empty directory")

    dataframe_dict = {
        dir_name: [_get_dataframes(os.path.join(tune_dir, dir_name))]
...
class ComparisonScalarRender(MultiScalarRender):
    ...
    def setup(self):
        ...
        for key in self.dataframe_dict.keys():
            self.selected_frames[key] = self.dataframe_dict[key]

    def default_check_boxes(self):
        for row_select, folder_name in zip(self.selector_widgets, self.dataframe_dict.keys()):
    ...
    def set_components(self):
        ...
        self.selector_widgets = [
            widgets.GridBox([
                ...
                widgets.Label(str(len(self.dataframe_dict[frames_name]))),
                widgets.Text(
                    "",
                    placeholder="Color",
                    layout=widgets.Layout(width="100px")),
            ], layout=row_layout)
            for frames_name in self.dataframe_dict.keys()]
    ...
    def render_figure(self):
        ...
        for ix, (folder_name, dataframe) in enumerate(self.selected_frames.items()):
            ...
            if re.match("#[0-9a-fA-F]{6}", color) is None:
                color = self.default_colors[ix % len(self.default_colors)]
